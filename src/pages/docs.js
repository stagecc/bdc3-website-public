...
const ViewSavedResultsLink = () => {
  ...
  return (
    <SavedResultsLink
      to={`${docSearchPath}/#saved`}
      aria-label="View saved search results"
      dim={savedResults.length === 0 ? 1 : 0}
    >
      <span className="link-text">View saved results</span>
      <span ref={wiggler}>
        {savedResults.length === 0 ? (
          <FolderIcon size={36} fill="var(--color-blueberry)" />
        ) : (
          <FolderFullIcon size={36} fill="var(--color-blueberry)" />
        )}
  ...
  return (
    <PageContent width="95%" maxWidth="1200px" center gutters>
      ...
      <DocSearch>
        ...
        >
          <Title style={{ flex: 1, border: 0, marginBottom: 0 }}>
