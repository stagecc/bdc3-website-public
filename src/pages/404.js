...
const Tray = styled.div`
    ...
    & ${ AnimatedNode } {
        ${ props => {
            ...
            return [...Array(props.length).keys()].map(i => `&:nth-child(${ i + 1 }) { animation-delay: ${ i * interval }ms; }`)
        }}
    }
`
...
const NotFoundAnimation = ({ color }) => {
    ...
    return (
        <Tray length={ notFoundMessage.length }>
            { notFoundMessage.split('').map(char => <AnimatedNode color={ color }>{ char }</AnimatedNode>) }
        </Tray>
    )
}
