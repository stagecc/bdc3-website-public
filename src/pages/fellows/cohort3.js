  ...
  return (
    <PageContent width="95%" maxWidth="1200px" center gutters>
      ...
      <section id="overview">
        ...
        <Paragraph>
          Cohort 3 Fellows will work on the BioData Catalyst Ecosystem from March 2021 to February 2022.
        </Paragraph>
      </section>
        ...
        <FellowsLinkList>
          {
            cohortThree.map(fellow => (
              <FellowsLinkListItem key={ kebabCase(fellow.name.replace(/,.+$/, '')) }
                path={ `/fellows/cohort3#${ kebabCase(fellow.name.replace(/,.+$/, '')) }` }
                text={ fellow.name }
              />
            ))
          }
...

        {
          cohortThree.map(fellow => (
            <FellowsProfile
              ...
              photo={ fellow?.photo?.childImageSharp?.fixed ? fellow.photo.childImageSharp.fixed : avatar.childImageSharp.fixed }
            />
          ))
        }
      </section>
