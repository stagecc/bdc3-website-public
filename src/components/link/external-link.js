...
export const ExternalLink = ({ to, asButton, noIcon, lightIcon, children, ...props }) => {
    ...
    const triggerDialog = event => {
        ...
        dialog.setContents(
            <div>
                <p>
                    This graphic notice ( <ExternalLinkIcon size={ 14 } fill="var(--color-blueberry)" /> )
    ...
    return requiresConfirmation
        ? (
            <a href={ to } onClick={ triggerDialog }>
                { children }
                { !noIcon && <ExternalLinkIcon fill={ lightIcon ? '#eee' : 'var(--color-blueberry)' } size={ 14 } { ...props } style={{ marginLeft: '0.25rem' }}/> }
            </a>
        )
        : (
