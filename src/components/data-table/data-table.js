...
const DownloadCSVButton = ({ onExport }) => (
  <DownloadButton small onClick={e => onExport(e.target.value)}>
    <DownloadIcon fill="var(--color-white)" size={16} /> CSV
  </DownloadButton>
);
const DownloadJSONButton = ({ onExport }) => (
  <DownloadButton small onClick={e => onExport(e.target.value)}>
    <DownloadIcon fill="var(--color-white)" size={16} /> JSON
  </DownloadButton>
);
...
export const DataTable = ({ columns, data, ...props }) => {
   ...
   */
  const isFiltered = (obj, key, value = "") =>
  ...
  const memoizedSubheaderComponent = useMemo(() => {
    ...
    return (
      <Fragment>
        <TextInput
          id="search-by-study-name"
          type="text"
          placeholder="Filter by Study Name"
          aria-label="Study Name Search Input"
          value={filters.Name}
        ...
        >
          <BackspaceIcon size={24} fill="var(--color-crimson)" />
        </IconButton>
      </Fragment>
    );
  }, [filters]);
  ...
  return (
    <Card style={fullscreen ? fullscreenStyle : null}>
      <CardHeader style={{ display: "flex", justifyContent: "flex-end" }}>
        ...
        >
          {pieChartVisible ? (
            <PieChartIcon size={24} fill="var(--color-white)" />
          ) : (
            <PieChartIcon size={24} fill="var(--color-crimson-dark)" />
          )}
        </IconButton>
        <IconButton
        ...
        >
          {fullscreen ? (
            <CloseIcon size={24} fill="var(--color-white)" />
          ) : (
            <FullscreenIcon size={24} fill="var(--color-white)" />
          )}
        </IconButton>
      </CardHeader>

      <CardBody
        style={{ overflowY: "auto", padding: 0, flex: 1, position: "relative" }}
      >
        {pieChartVisible && (
          <PieChart
            title={
