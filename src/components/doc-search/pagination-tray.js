...
const PaginationIconButton = ({ icon, clickHandler, disabled }) => {
  ...
  return (
    <IconButton disabled={disabled} onClick={disabled ? null : clickHandler}>
      <PaginationIcon
        size={24}
        fill={disabled ? "var(--color-lightgrey)" : "var(--color-crimson)"}
      />
    </IconButton>
  );
};
    ...
    <Wrapper aria-label="pagination">
      <div className="actions">
        <PaginationIconButton
          ...
          clickHandler={handleGoToPreviousPage}
          disabled={currentPage <= 1}
          aria-disabled={currentPage <= 1}
        />
        <EllipsisIcon
          fill={currentPage > paginationRadius + 1 ? "#ccc" : "transparent"}
          size={24}
        />
        &nbsp;&nbsp;
        {[...Array(pageCount).keys()].map(i => {
          ...
          if (currentPage <= paginationRadius) {
            [minPage, maxPage] = [1, 2 * paginationRadius + 1];
          }

          // are we near the end?
          if (currentPage >= pageCount - paginationRadius) {
            [minPage, maxPage] = [pageCount - 2 * paginationRadius, pageCount];
          }
        ...
        <EllipsisIcon
          fill={
            pageCount > 2 * paginationRadius - 1 &&
        ...
        <PaginationIconButton
          ...
          clickHandler={handleGoToNextPage}
