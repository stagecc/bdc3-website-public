  ...
  const MemoizedSaveButton = useMemo(
    () => (
      <Actions selected={alreadySaved}>
        <p
          role="tooltip"
          id={`${result.cacheId}-tooltip`}
          data-tip={
            alreadySaved
              ? "Remove this result from my folder"
              : "Add this result to my folder"
          }
        >
          {alreadySaved ? (
            <IconButton
              onClick={() => removeResult(result)}
              aria-describedby={`${result.cacheId}-tooltip`}
            >
              <DocumentRemoveIcon fill="var(--color-white)" size={24} />
            </IconButton>
          ) : (
            <IconButton
              ...
              aria-describedby={`${result.cacheId}-tooltip`}
            >
              <DocumentAddIcon fill="var(--color-white)" size={24} />
            </IconButton>
          )}
        </p>
        ...
        <Content>
          ...
          <p
            dangerouslySetInnerHTML={{
              __html: htmlSnippet.replace(/<br>/g, "").replace(/n?\\n/gm, "")
            }}
          />
        </Content>
        <Visible lg xl>
