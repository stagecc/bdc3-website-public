...
export const useFreshdeskFaqs = category => {
    ...
    useEffect(() => {
        ...
        const fetchArticles = async () => {
            await axios.get(`${ FRESHDESK_API_ROOT }/solutions/categories/${ categoryId }/folders`, requestOptions)
                .then(response => {
                    const folders = response.data.sort((f, g) => f.name > g.name)
                    const articlePromises = folders.map(folder => axios.get(`${ FRESHDESK_API_ROOT }/solutions/folders/${ folder.id }/articles`, requestOptions)
                        .then(response => {
